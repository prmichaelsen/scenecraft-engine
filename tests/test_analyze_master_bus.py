...
``compute_mix_graph_hash`` is monkeypatched to a fixed value per test so
rendered-path resolution is deterministic. The real hash function is
exercised separately in ``test_mix_graph_hash.py``.
...
def pin_mix_graph_hash(monkeypatch):
    """Force ``compute_mix_graph_hash`` to return STUB_HASH so the rendered-
    path (``pool/mixes/<hash>.wav``) resolves to a fixed location per test.
    The real hash function is exercised in test_mix_graph_hash.py.
    """
...
def test_rms_and_centroid_datapoints_are_persisted(project):
    from scenecraft.db_mix_cache import query_mix_datapoints
    ...
    rms_rows = query_mix_datapoints(project, run_id, "rms")
    cent_rows = query_mix_datapoints(project, run_id, "spectral_centroid")
    assert len(rms_rows) > 10
    assert len(cent_rows) > 10
    # RMS should hover around 0.5 / sqrt(2) ≈ 0.354 for a 0.5-amp sine.
    mid_rms = np.median([d.value for d in rms_rows])
...
def test_force_rerun_replaces_old_run(project):
    ...
    # Old run should be gone from mix_analysis_runs; new run present.
    conn = get_db(project)
    rows = {r["id"] for r in conn.execute("SELECT id FROM mix_analysis_runs").fetchall()}
    assert first_id not in rows
    assert second["run_id"] in rows
