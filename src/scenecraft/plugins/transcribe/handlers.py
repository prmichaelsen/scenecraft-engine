...
# ── REST adapter handlers (manifest contributes.restEndpoints) ─────────
#
# These are thin shims over the same transcriber.* library functions the
# chat-tool handlers use. They match the signature the plugin-host
# dispatcher passes to REST handlers:
#
#   handler(path, project_dir, project_name, body_or_query, *, path_groups=None)
#
# `path_groups` arrives as a dict only when the registered regex has named
# groups — for the `/runs/(?P<run_id>[^/]+)` endpoint we use it to pick the
# run id out of the URL. Other endpoints ignore the kwarg.
...
def handle_rest_run(
    ...
    **_kwargs,
    ...
    """POST /api/projects/:name/plugins/transcribe/run — trigger a run."""
    del path, project_name  # unused
    body = body or {}
    clip_id = body.get("clip_id") or body.get("clipId")
    if not clip_id or not isinstance(clip_id, str):
        return {"error": "Missing 'clip_id'"}
...
def handle_rest_list_runs(
    ...
    **_kwargs,
    ...
    """GET /api/projects/:name/plugins/transcribe/runs?clip_id=<opt>."""
    del path, project_name
    clip_id = None
    if isinstance(query, dict):
        # query values arrive as lists from urllib's parse_qs
        raw = query.get("clip_id") or query.get("clipId")
        if isinstance(raw, list) and raw:
            clip_id = raw[0]
        elif isinstance(raw, str):
            clip_id = raw
    try:
        runs = transcriber.list_runs(project_dir, clip_id=clip_id or None)
    except Exception as exc:  # noqa: BLE001
        return {"error": f"{type(exc).__name__}: {exc}"}
    return {"runs": runs, "count": len(runs)}


def handle_rest_get_run(
    path: str,
    project_dir: Path,
    project_name: str,
    query: dict,
    ...
    **_kwargs,
    ...
    run_id = (path_groups or {}).get("run_id")
    if not run_id:
        return {"error": "run_id missing from URL"}
    try:
        result = transcriber.get_run(project_dir, run_id)
    except Exception as exc:  # noqa: BLE001
        return {"error": f"{type(exc).__name__}: {exc}"}
